...
- Caminho do squad: `.synapos/squads/{slug}/`
...
.synapos/squads/{slug}/_memory/memories.md → aprendizados anteriores
.synapos/_memory/company.md               → perfil da empresa/usuário
.synapos/_memory/preferences.md           → preferências de saída
```
...
steps:
  - id: step-id
    ...
    output_file: docs/{nome}.md       # opcional
...
Crie: `.synapos/squads/{slug}/output/{YYYY-MM-DD-HHmmss}/`
...
Se `output_file` definido:
- Crie o arquivo na **raiz do projeto**: `{output_file}` (ex: `docs/spec.md`)
- Crie cópia de histórico em `.synapos/squads/{slug}/output/{run_id}/{output_file}`
...
Salve cópia de state.json em `.synapos/squads/{slug}/output/{run_id}/state.json`.
...
Se houver resposta, adicione em `.synapos/squads/{slug}/_memory/memories.md`:
```markdown
...
Run salvo em: .synapos/squads/{slug}/output/{run_id}/
...
3. **Memória do squad** (memories.md — preferências e aprendizados)
4. **Outputs anteriores relevantes** (definidos em `depends_on`)
5. **Instruções do step** (arquivo do step)
6. **Base path do squad** (caminho absoluto para todas as operações de arquivo)

A ordem de composição sempre é:
```
[Agent Persona] + [Contexto Squad] + [Memória] + [Outputs Anteriores] + [Instrução do Step]
```
...
```
IMPORTANTE — CAMINHOS DE ARQUIVO:
- Documentos gerados (docs/):  raiz do projeto (ex: docs/spec.md)
- Memória do squad:            .synapos/squads/{slug}/_memory/
- Histórico de execução:       .synapos/squads/{slug}/output/{run_id}/

Qualquer referência a "docs/" nas instruções do step
significa a pasta "docs/" na raiz do projeto.
```
